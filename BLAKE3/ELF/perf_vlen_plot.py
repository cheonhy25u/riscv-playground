...
# ✅ CSV 파일 로드
df = pd.read_csv("perf_results_b3.csv")
...
# ✅ 실행 파일별 필터링
df_x86 = df[df["ExecName"] == "./b3_x86"]
df_x86_avx2 = df[df["ExecName"] == "./b3_avx2"]
df_riscv_normal = df[(df["ExecName"] == "spike -m8192 --isa=rv64gcv --disable-dtb --rbb-port=0 --log-commits=0 ./b3_riscv")]
df_riscv_vtype = df[df["ExecName"].str.contains("b3_riscv_v") & (df["VLEN"] != "N/A")]
...
for metric, ylabel in metrics.items():
    ...
    plt.xscale("log")
    plt.savefig(f"{metric}_vs_FileSizeMB_BLAKE3.png", dpi=300)
    plt.close()

print(f"✅ 최적 VLEN: {best_vlen}")
print("✅ 그래프 생성 완료! *_vs_FileSizeMB_BLAKE3.png 파일 확인")
